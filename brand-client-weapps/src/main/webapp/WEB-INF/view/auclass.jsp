...
<c:if test="${not empty largeType}">
	<title>澳洲${largeType.catalog}类  ${largeType.name}</title>
</c:if>
<c:if test="${empty largeType}">
	<title>澳洲商标注册用《商品与服务分类表》</title>
</c:if>
...
<body>
	<input type="hidden" id="currentNavId" value="auclass">
								...
								<div class="mod-banner">
									<h3>
										<span style="color: #ffffff;"> <a
											href="${ctxPath}/auclass.html" target="_self"><span
												style="color: #ffffff;">澳洲商品与服务区分表</span></a>
										</span>
									</h3>
								</div>
							</div>
			...
			<div class="container">
				<div class="">
					<ul class="breadcrumb">
						...
						<c:if test="${empty largeType}">
							<li class="active"><span>澳洲商品与服务项目</span></li>
						</c:if>
						<c:if test="${not empty largeType and empty smallType}">
							<li><a href="${ctxPath}/auclass.html" class="pathway">澳洲商品与服务项目</a></li>
							<li class="active"><span>${largeType.name}</span></li>
						</c:if>
						<c:if test="${not empty smallType and not empty largeType}">
							<li><a href="${ctxPath}/auclass.html" class="pathway">澳洲商品与服务项目</a></li>
							<li><a href="${ctxPath}/auclass/class${largeType.id}.html" class="pathway">${largeType.name}</a></li>
						...
						<div class="module-inner">
							<div class="module-ct">
								<div class="custom">
									<blockquote>
										<c:if test="${not empty largeType and empty smallType}">
											<b>澳大利亚商标注册用《商品和服务分类表》使用说明</b><br/>
											本分类表最后更新于2019年1月。<br/>
											NC——为国际尼斯分类中包含的术语<br/>
											AP——为澳大利亚官方公布的商品或服务描述术语<br/>
										</c:if>
				...
				<div id="t3-content" class="t3-content col-xs-8 col-xs-push-4">
					<div class="cbListView">
						<form action="" method="post" name="adminForm" id="adminForm"
							...
							<c:if test="${not empty largeType}">
								<h1 class="contentheading">澳洲${largeType.catalog}类  ${largeType.name}</h1>
							</c:if>
							...
							<div id="cbSearchButtonWrap"
								...
								<button class="button btn btn-secondary cbButton cbSearchButton"
									onclick="getCookieList('auclass');" data-toggle="modal" data-target="#cookieList">获取标记值</button>
							</div>
						...
						<table class="category table table-striped" width="100%" border="0" cellspacing="0" cellpadding="2" style="font-size:14px;margin-top: 50px;">
							<thead>
								<tr>
									...
									<th class="sectiontableheader hidden-phone" style="min-width:50px;">源</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${smallTypeItemList}" var="item" varStatus="index">
									<tr class="row${index.index/2}">
										...
										<td class="hidden-phone">${item.projectSource}</td>
										</td>
									</tr>
								</c:forEach>
							</tbody>
						...
						<script type="text/javascript">
							...
							//设置值
							function setAuClassCookie(obj,id,name){
								if($(obj).is(':checked')){
									$.ajax({ 
										url: "${ctxPath}/setAuClassCookie",
										type: 'POST',
									...
									$.ajax({ 
										url: "${ctxPath}/cancelAuClassCookie/"+id,
										type: 'POST',
										success: function(){
								    	}
									});
											...
											<div class="panel-heading">
												<h4>
													<span style="font-size: 16pt;"><a
														href="${ctxPath}/auclass.html" target="_self">澳洲《商品与服务区分表》</a>目录</span>
												</h4>
											</div>
														...
														<c:forEach items="${largeTypeList}" var="item">
															<label style="width:28px;text-align: center;">
																<span style="font-family: arial black,avant garde; font-size: 12pt;">
																	<span style="font-family: arial black,avant garde; font-size: 12pt;">
																		<a href="${ctxPath}/auclass/class${item.id}/">${item.catalog}</a>
																	</span>
																</span>
															</label>
														</c:forEach>
											...
											<div class="panel-heading">
												<h4 class="panel-title" style="text-align: left;">
													...
													<span style="color: #21c2f8; font-size: 10pt;"> 
														<a href="${ctxPath}/auclass/class${item.id}/" target="_self"> 
