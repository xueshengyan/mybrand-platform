...
<title>${navItem.cityName}商标注册用《商品与服务分类表》</title>
...
<body>
	<input type="hidden" id="currentNavId" value="${navItem.url}${navItem.id}">
								...
								<div class="mod-banner">
									<h3>
										<span style="color: #ffffff;"> <a
											href="${ctxPath}/dycclass${navItem.id}.html" target="_self"><span
												style="color: #ffffff;">${navItem.cityName}商品与服务区分表</span></a>
										</span>
									</h3>
								</div>
							</div>
				...
				<div class="">
					<ul class="breadcrumb">
						...
						<c:if test="${empty largeType}">
							<li class="active"><span>${navItem.cityName}商品与服务项目</span></li>
						</c:if>
						<c:if test="${not empty largeType and empty smallType}">
							<li><a href="${ctxPath}/dycclass${navItem.id}.html" class="pathway">${navItem.cityName}商品与服务项目</a></li>
							<li class="active"><span>${largeType.name}</span></li>
						</c:if>
						<c:if test="${not empty smallType and not empty largeType}">
							<li><a href="${ctxPath}/dycclass${navItem.id}.html" class="pathway">${navItem.cityName}商品与服务项目</a></li>
							<li><a href="${ctxPath}/dycclass${navItem.id}/class${largeType.id}.html" class="pathway">${largeType.name}</a></li>
							...
							<div class="module-ct">
								<div class="custom">
									...
									<c:if test="${empty largeType and empty smallType}">
										<blockquote>
										<b>${navItem.cityName}《商品和服务分类表》使用说明</b><br/>
										本分类表最后更新于2019年1月。<br/>
										${navItem.cityName}商标注册用分类表采用单层直铺式设计，可通过类别或检索器直接选择适合的商品或服务描述。<br/>
					...
					<div class="cbListView">
						<form action="" method="post" name="adminForm" id="adminForm"
							...
							<c:if test="${not empty largeType}">
								<h1 class="contentheading">${navItem.cityName}${largeType.catalog}类  ${largeType.name}</h1>
							</c:if>
							...
							<div id="cbSearchButtonWrap"
								...
								<button class="button btn btn-secondary cbButton cbSearchButton"
									onclick="getCookieList('dycclass${navItem.id}');" data-toggle="modal" data-target="#cookieList">获取标记值</button>
							</div>
								...
								<c:forEach items="${smallTypeItemList}" var="item" varStatus="index">
									<tr class="row${index.index/2}">
										<td>
											...
											<c:forEach items="${dycCookieList}" var="ck">
												<c:if test="${ck.id eq item.id}">
													<input type="checkbox" value="" checked="checked" id="cookie_checkbox_${item.id}"
													onclick="setDycClassCookie(this,${item.id},'${item.projectName}-${item.projectTranslation}',${navItem.id});">
										...
										<td>
											...
											onclick="detail('${item.typeNo}','${item.projectName}','${item.projectTranslation}','${item.projectStatus}','${item.projectInvalidDate}','${item.projectType}','${item.projectVersion}');">
											${item.typeNo}</a>
										</td>
						...
						<script type="text/javascript">
							...
							//设置值
							function setDycClassCookie(obj,id,name,navItemId){
								if($(obj).is(':checked')){
									$.ajax({ 
										url: "${ctxPath}/setDycClassCookie-"+navItemId,
										type: 'POST',
									...
									$.ajax({ 
										url: "${ctxPath}/cancelDycClassCookie/"+id+"/"+navItemId,
										type: 'POST',
										success: function(){
								    	}
									});
							...
							function delCookie(id,obj,name){
								$.ajax({ 
									url: "${ctxPath}/cancelDycClassCookie/"+id+"/${navItem.id}",
									type: 'POST',
									success: function(){
										$(obj).parent().remove();
											...
											<div class="panel-heading">
												<h4>
													<span style="font-size: 16pt;"><a
														href="${ctxPath}/dycclass${navItem.id}.html" target="_self">${navItem.cityName}《商品与服务区分表》</a>目录</span>
												</h4>
											</div>
														...
														<c:forEach items="${largeTypeList}" var="item">
															<label style="width:28px;text-align: center;">
																<span style="font-family: arial black,avant garde; font-size: 12pt;">
																	<span style="font-family: arial black,avant garde; font-size: 12pt;">
																		<a href="${ctxPath}/dycclass${navItem.id}/class${item.id}/">${item.catalog}</a>
																	</span>
																</span>
															</label>
														</c:forEach>
											...
											<div class="panel-heading">
												<h4 class="panel-title" style="text-align: left;">
													...
													<span style="color: #21c2f8; font-size: 10pt;"> 
														<a href="${ctxPath}/dycclass${navItem.id}/class${item.id}/" target="_self"> 
...
<script type="text/javascript">
	function detail(row1,row2,row3,row4,row5,row6,row7){
		$('#row1').text(row1);
