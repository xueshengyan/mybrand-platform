...
<c:if test="${not empty largeType}">
	<title>日本${largeType.catalog}类  ${largeType.name}</title>
</c:if>
<c:if test="${empty largeType}">
	<title>日本商标注册用《商品与服务分类表》</title>
</c:if>
...
<body>
	<input type="hidden" id="currentNavId" value="jpclass">
								...
								<div class="mod-banner">
									<h3>
										<span style="color: #ffffff;"> <a
											href="${ctxPath}/jpclass.html" target="_self"><span
												style="color: #ffffff;">日本商品与服务区分表</span></a>
										</span>
									</h3>
								</div>
							</div>
			...
			<div class="container">
				<div class="">
					<ul class="breadcrumb">
						...
						<c:if test="${empty largeType}">
							<li class="active"><span>日本商品与服务项目</span></li>
						</c:if>
						<c:if test="${not empty largeType and empty smallType}">
							<li><a href="${ctxPath}/jpclass.html" class="pathway">日本商品与服务项目</a></li>
							<li class="active"><span>${largeType.name}</span></li>
						</c:if>
						<c:if test="${not empty smallType and not empty largeType}">
							<li><a href="${ctxPath}/jpclass.html" class="pathway">日本商品与服务项目</a></li>
							<li><a href="${ctxPath}/jpclass/class${largeType.id}.html" class="pathway">${largeType.name}</a></li>
						...
						<div class="module-inner">
							<div class="module-ct">
								<div class="custom">
									<blockquote>
										<c:if test="${not empty largeType and empty smallType}">
											<b>日本《商品和服务分类表》使用说明</b><br/>
											本日本商标注册用分类表最后更新于2019年1月。<br/>
		...
		<div id="t3-mainbody" class="container t3-mainbody sidebar-left">
			<div class="row">
				...
				<div id="t3-content" class="t3-content col-xs-8 col-xs-push-4">
					<div class="cbListView">
						<form action="" method="post" name="adminForm" id="adminForm"
							...
							<c:if test="${not empty largeType}">
								<h1 class="contentheading">日本${largeType.catalog}类  ${largeType.name}</h1>
							</c:if>
							...
							<div id="cbSearchButtonWrap"
								...
								<button class="button btn btn-secondary cbButton cbSearchButton"
									onclick="getCookieList('jpclass');" data-toggle="modal" data-target="#cookieList">获取标记值</button>
							</div>
						...
						<script type="text/javascript">
							...
							//设置值
							function setJpClassCookie(obj,id,name){
								if($(obj).is(':checked')){
									$.ajax({ 
										url: "${ctxPath}/setJpClassCookie",
										type: 'POST',
									...
									$.ajax({ 
										url: "${ctxPath}/cancelJpClassCookie/"+id,
										type: 'POST',
										success: function(){
								    	}
									});
				...
				<div class="t3-sidebar t3-sidebar-left col-xs-4 col-xs-pull-8">
					<div class="t3-module module-jpclass" id="Mod226">
						<div class="module-inner">
							<div class="module-ct">
								<div class="custom-jpclass">
									<div id="accordion" class="panel-group">
										<div class="panel panel-default">
											<div class="panel-heading">
												<h4>
													<span style="font-size: 16pt;"><a
														href="${ctxPath}/jpclass.html" target="_self">日本《商品与服务区分表》</a>目录</span>
												</h4>
											</div>
														...
														<c:forEach items="${largeTypeList}" var="item">
															<label style="width:28px;text-align: center;">
																<span style="font-family: arial black,avant garde; font-size: 12pt;">
																	<span style="font-family: arial black,avant garde; font-size: 12pt;">
																		<a href="${ctxPath}/jpclass/class${item.id}/">${item.catalog}</a>
																	</span>
																</span>
															</label>
														</c:forEach>
											...
											<div class="panel-heading">
												<h4 class="panel-title" style="text-align: left;">
													...
													<span style="color: #21c2f8; font-size: 10pt;"> 
														<a href="${ctxPath}/jpclass/class${item.id}/" target="_self"> 
